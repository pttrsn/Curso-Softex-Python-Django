...
def cadastrar_localidade(bairros: dict) -> None:
    ...
    nome_bairro = input('Digite o nome do bairro (identificando) ').lower()
...
def processar_pedido(paes_disponiveis: dict) -> tuple[dict, int, float, dict]| None:
    ...
    Retorna uma tupla com o dicionário de pães, quantidade,
...
def iniciar_programa() -> None:
    ...
    codigo_vendas = banco_dados['codigo_venda_base']

    while True:
        print(f'-- Bem-vindo(a) à Padaria Desespero, sou o(a) atendente {atendente}.')
        ...
        if opcao == '1':
            ...
            pao_escolhido, quantidade_pedido, valor_compra, paes_estoque = pedido

            print(f'Seu pedido foi de {quantidade_pedido}{pao_escolhido['nome']} e ficou em R$ {valor_compra:.2f}')

            forma_retirada = input('É para 1. Retirar ou 2. Entregar?: ')
            valor_frete = 0.0

            if forma_retirada == "2":
                entrega = calcula_frete(bairros_disponiveis)
            ...
            cod_venda = gerar_codigo_venda(codigo_vendas)
            ...
            print('Erro! Entrada de opção inválida.')
